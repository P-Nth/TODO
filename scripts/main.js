...
function addTask() {
    ...
    if (taskText !== '') {
      ...
      // Toggle the "remove" class on the respective placeholders
      const currentPlaceholder = document.querySelector('.current_pl');
      const completedPlaceholder = document.querySelector('.completed_pl');
      const editedPlaceholder = document.querySelector('.edited_pl');
      const deletedPlaceholder = document.querySelector('.deleted_pl');
  
      if (tasks.length === 1) {
        currentPlaceholder.classList.add('remove');
      }
  
      if (completedTasks.length === 1) {
        completedPlaceholder.classList.add('remove');
      }
  
      if (editedTasks.length === 1) {
        editedPlaceholder.classList.add('remove');
      }
  
      if (deletedTasks.length === 1) {
        deletedPlaceholder.classList.add('remove');
      }
    }
}
...
function renderTasks() {
    ...
    if (tasks.length > 0) {
      const maxCurrentTasks = Math.min(tasks.length, 8);
      for (let i = 0; i < maxCurrentTasks; i++) {
    ...
    if (completedTasks.length > 0) {
      ...
      for (let i = 0; i < maxCompletedTasks; i++) {
    ...
    if (deletedTasks.length > 0) {
      ...
      for (let i = 0; i < maxDeletedTasks; i++) {
    ...
    if (editedTasks.length > 0) {
      ...
      for (let i = 0; i < maxEditedTasks; i++) {
...
/**
------ Initial ------
🚀 Embarking on a journey to brush up my vanilla JavaScript skills! Building a Todo app to challenge myself and see if I can still tackle basic tasks with JS. Excited to dive back into the fundamentals! Stay tuned for updates on my progress! #webdevelopment #TodoApp #JavaScript

------ Next ------
🚀 Building a sleek and functional Todo app with HTML, CSS, and JavaScript! Adding task management features, styling with CSS, and implementing interactivity with JS. Can't wait to share the final result soon! Stay tuned! #webdevelopment #TodoApp #HTML #CSS #JavaScript

----- Final -----

🎉 Excited to share my newly built Todo app using HTML, CSS, and JavaScript! 📝✨ Experience task management with ease – add, edit, and delete tasks effortlessly, complemented with intuitive icons. Persistence is ensured with local storage. #webdevelopment #TodoApp #HTML #CSS #JavaScript"

Give it a try here: [insert link to your app] 
